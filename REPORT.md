...
3. **路网未完全连通（图碎片化，实为建图伪影）**：最大连通分量占比 Vienna 77.7%、HK 59.9%，地图上街道并非处处相连。经核查，这**几乎全是建图启发式的伪影、而非真实路网断裂**：OSM 数据自带 `u`/`v` 拓扑节点 id，**直接用 u/v 建图两城均为 100% 连通（单一连通分量）**；而本文 `build_road_graph_edges` 丢弃 u/v、改以"几何端点 10 m 平面吸附"重建连通，才人为切碎路网。香港退化更严重（59.9% vs 77.7%）的原因：香港 **bridge 标签占 6.1%，是维也纳（2.1%）的 3 倍**，多层立交、高架、匝道密集——高架与地面端点平面接近却不同层、匝道端点错位、多边交汇采样错开，端点吸附启发式失败率显著更高。碎片在地理上嵌于最大连通块内部（最大块已覆盖全港范围），并非岛屿隔离。叠加 Stage 6 主动按边界断边并过滤 <3 节点 / <50 m / <3 街景点的小单元，进一步形成视觉空隙。**改进**：建图时优先使用 OSM `u`/`v` 拓扑（按共享节点 id 连接路端点），无 u/v 时再回退到容差吸附——预计两城连通率可恢复至接近 100%。
