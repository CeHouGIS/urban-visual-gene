...
1. **结果强烈受采样限制**：样本 4×，覆盖率约 3.4×、真信号边约 2.6×、全图单元 4–7×——所有绝对量随 $N$ 单调增长（图 a–c），说明小样本下的单元数并非城市的稳定属性。
2. **全图分割仍被插值主导**：即便 $N=2000$，激活余弦距离非零的边仅占 6–7%（图 b），其余 90%+ 为插值复制（$d_{ij}\approx0$）。故应在覆盖子图上、而非插值全图上得出结论。
3. **覆盖单元密度近似采样不变**：`覆盖单元 / 覆盖节点` 在两城内部跨 $N$ 基本恒定（Vienna ≈0.077、香港 ≈0.066，图 d），且两城取值不同——这是首个不随样本量漂移的描述量，宜作为**归一化的城市级指标**，替代易受采样影响的绝对单元数（亦回应"两城比较需标准化"的意见）。

> 注：本节为评审 Major-2（数据规模）的定量回应；受本机环境对大样本的稳定性限制，$N=5000$ 仍在补算。
