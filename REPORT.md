...
3. **路网未完全连通（图碎片化）**：最大连通分量占比 Vienna 77.7%、HK 59.9%，故地图上街道并非处处相连。主因有三：(i) **路口连接判据偏严**——`intersection_connect` 仅当两条道路的**端点**落在 10 m 容差内才连边，而 OSM 中大量交叉发生在道路**中间顶点**（T/十字路口的被交路常以非端点顶点相接），这类路口被漏连；(ii) 路网按采样点包络裁剪，边界处道路被截断；(iii) 真实地物本就不连通——跨海/跨山的桥隧、立交、轮渡、独立步行路。香港更碎（59.9%）源于其多山多水多岛、立交与水体分隔显著，且 500 随机点的包络跨新界/离岛留有空隙。叠加 Stage 6 主动按边界断边、并过滤掉 <3 节点 / <50 m / <3 街景点的小单元，进一步在视觉上形成空隙。**改进**：路口连边改为对**全部顶点**做容差吸附（而非仅端点）、或增大容差并显式处理桥隧层级（OSM `layer`/`bridge` 标签）。
