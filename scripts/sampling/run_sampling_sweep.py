...
the full-graph and covered-only segmentation. Produces outputs/sweep/summary.csv.
  ...
  python -m scripts.sampling.run_sampling_sweep --city Vienna --sizes 500 2000 5000
...
def run_one(city: str, n: int, K: int, epochs: int) -> dict:
    ...
    print(f"\n===== {city}  N={n} =====", flush=True)
    _run("scripts.pipeline.run_stage1", "--city", city, "--max-panos", str(n), "--out", str(out))
    _run("scripts.pipeline.run_stage2", "--city", city, "--out", str(out))
    _run("scripts.pipeline.run_stage3", "--out", str(out))
    _run("scripts.pipeline.run_stage45", "--out", str(out), "--K", str(K), "--epochs", str(epochs))

    _run("scripts.pipeline.run_stage6", "--out", str(out))
    r6_full = _report(out, "stage6")
    _run("scripts.pipeline.run_stage6", "--out", str(out), "--covered-only")
