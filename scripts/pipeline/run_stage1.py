...
geopandas/scipy stages (OpenMP/MKL clash → native segfaults).
  ...
  python -m scripts.pipeline.run_stage1 --city HongKong --max-panos 500
"""
...
from scripts.core.cities import (img_root, img_root_fallback, load_panos, out_dir,
                            path_style)
from scripts.core.io_utils import save_report
from scripts.pipeline.stage1_extract_pano_features import extract_pano_features
...
def main():
    ...
    ap.add_argument("--sampling", choices=["random", "greedy"], default="random",
                    help="greedy = use scripts.sampling.select_panos max-coverage set")
    args = ap.parse_args()
    ...
    if args.exclude_bad:
        from scripts.quality.image_quality import filter_panos
