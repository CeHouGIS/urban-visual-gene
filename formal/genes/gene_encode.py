...
from formal.gpu_run import Extractor, SAE, stratified_panos, imgpath, CITY_DIR
...
for city in CITIES:
    ...
    for pid in on:
        ...
        feats,_=ext.batch([im]); zt=feats[0].to(dev)          # (784,1024)
        with torch.no_grad():
        ...
        im.resize((224,224)).save(TH/f"{gi}.jpg",quality=80); meta.append((city,pid)); gi+=1
    log(f"{city}: {sum(1 for m in meta if m[0]==city)} encoded")
